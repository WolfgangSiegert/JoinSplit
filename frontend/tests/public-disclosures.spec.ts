...
test('shows the public-demo boundary before data entry and keeps full disclosures reachable', async ({ page }) => {
  ...
  await expect(page.getByText('Bereits erfolgreich geladene Ansichten können über den begrenzten App-Shell-Cache offline neu starten.')).toBeVisible()
  await expect(page.getByText('Ein erster Aufruf oder eine zuvor nicht geladene Ansicht ist ohne Netzwerk weiterhin nicht garantiert.')).toBeVisible()
