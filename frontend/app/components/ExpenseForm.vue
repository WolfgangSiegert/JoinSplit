...
<template>
  <form class="space-y-5" @submit.prevent="submit">
    ...
    <section class="card p-4" aria-labelledby="add-during-expense" :aria-busy="addingParticipant"><h2 id="add-during-expense" class="font-semibold">Weitere Person hinzufügen</h2><div class="mt-2 flex flex-col gap-2 sm:flex-row"><label for="draft-participant" class="sr-only">Name der neuen Person</label><input id="draft-participant" ref="addInput" v-model="addName" class="field-input" :disabled="addingParticipant || submitting" :aria-invalid="Boolean(addError)" :aria-describedby="addError ? 'draft-participant-error' : undefined"><button type="button" class="secondary-button shrink-0" :disabled="addingParticipant || submitting" @click="addParticipant()">{{ addingParticipant ? 'Wird hinzugefügt …' : 'Hinzufügen' }}</button></div><p v-if="addError" id="draft-participant-error" class="error-text mt-2">{{ addError }}</p><div v-if="duplicateParticipantName" class="mt-3 rounded-lg bg-amber-50 p-3 text-amber-950" role="alert"><p class="font-semibold">Name bereits vorhanden</p><p class="mt-1">Der Name „{{ duplicateParticipantName }}“ wird in dieser Gruppe bereits verwendet.</p><button ref="duplicateConfirmButton" type="button" class="secondary-button mt-3" :disabled="addingParticipant || submitting" @click="addParticipant(true)">Trotzdem hinzufügen</button></div></section>
