...
async function addParticipant() {
  ...
  } catch { status.value = 'Die Person konnte nicht lokal gespeichert werden.' }
}
</script>
...
<template>
  <form class="space-y-5" @submit.prevent="submit">
    ...
    <fieldset ref="sharesFieldset" :aria-describedby="errors.participantIds ? 'shares-help shares-error' : 'shares-help'" :aria-invalid="Boolean(errors.participantIds)"><legend class="font-semibold">Gleichmäßig aufteilen</legend><p id="shares-help" class="mt-1 text-sm text-gray-600">Die Reihenfolge der Gruppe bestimmt, wer Rest-Cents erhält.</p><div class="mt-3 space-y-2"><label v-for="participant in participants" :key="participant.id" class="flex min-h-11 items-center gap-3"><input v-model="draft.participantIds" type="checkbox" :value="participant.id" :disabled="participant.status === 'inactive' && !expense?.shares.some(share => share.participantId === participant.id)"><span>{{ participant.name }}{{ participant.status === 'inactive' ? ' (inaktiv)' : '' }}</span></label></div><p v-if="errors.participantIds" id="shares-error" role="alert" class="error-text mt-2">{{ errors.participantIds }}</p></fieldset>
    <section class="card p-4" aria-labelledby="split-preview"><h2 id="split-preview" class="text-lg font-semibold">Vorschau der Aufteilung</h2><p v-if="!preview.length" class="mt-2 text-gray-600">Betrag und mindestens eine Person auswählen.</p><ul v-else class="mt-3 space-y-2"><li v-for="share in preview" :key="share.participantId" class="flex justify-between gap-4"><span>{{ names.get(share.participantId) }}</span><strong>{{ formatAmountMinor(share.amountMinor) }}</strong></li></ul></section>
    <section class="card p-4" aria-labelledby="add-during-expense"><h2 id="add-during-expense" class="font-semibold">Weitere Person hinzufügen</h2><div class="mt-2 flex flex-col gap-2 sm:flex-row"><label for="draft-participant" class="sr-only">Name der neuen Person</label><input id="draft-participant" ref="addInput" v-model="addName" class="field-input" :aria-invalid="Boolean(addError)" :aria-describedby="addError ? 'draft-participant-error' : undefined"><button type="button" class="secondary-button shrink-0" @click="addParticipant">Hinzufügen</button></div><p v-if="addError" id="draft-participant-error" class="error-text mt-2">{{ addError }}</p></section>
    ...
    <button type="submit" class="primary-button w-full" :disabled="submitting">{{ expense ? 'Änderungen speichern' : 'Ausgabe speichern' }}</button>
  </form>
</template>
