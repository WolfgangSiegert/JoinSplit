...
export async function synchronizeExpenseMutation(options: Options): Promise<ExpenseSyncResult> {
  ...
  try {
    ...
    response = await fetcher(url, { method: mutation.type === 'CreateExpense' ? 'POST' : mutation.type === 'UpdateExpense' ? 'PUT' : 'DELETE', headers: context.headers, credentials: context.credentials, ...(body ? { body: JSON.stringify(body) } : {}) })
